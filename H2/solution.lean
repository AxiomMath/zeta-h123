...
theorem tset_value_nonempty (q d k : ℕ) (hq : 2 ≤ q) (hk : 0 < k) :
  ...
  · intro e
    ...
    have hmd : ∀ i : Fin d, ((fun x => qdigit q x e) ∘ m) i = if e = k + i.val then q - 1 else 0 := by
...
def slotLen (q d k : ℕ) : ℕ := k + d * (k + 1) + 1
...
theorem qdigit_zero_of_ge (q k1 e : ℕ) (hq : 2 ≤ q) (he : k1 + 1 ≤ e) :
  ...
  have : (Nat.digits q k1).length ≤ k1 + 1 := by
    ...
    · simp
    · rw [Nat.digits_len q k1 hq hk0]; have := Nat.log_le_self q k1; omega
  omega
...
theorem greedy_core (q p L : ℕ) (hq : 2 ≤ q) (hpL : p ≤ L)
  ...
  have hsplit : Finset.range L = Finset.range p ∪ Finset.Ico p L := by
    rw [Finset.range_eq_Ico, Finset.Ico_union_Ico_eq_Ico (by omega) hpL]
  ...
  have hcountIco : (∑ e ∈ Finset.range p, (s e - b e)) + r ≤ ∑ e ∈ Finset.Ico p L, b e := by
    have hb_le_s : ∑ e ∈ Finset.range p, b e ≤ ∑ e ∈ Finset.range p, s e := by
      apply Finset.sum_le_sum; intro e he; simp only [Finset.mem_range] at he; exact hbs e (by omega)
    have hsubsum : ∑ e ∈ Finset.range p, (s e - b e)
        = (∑ e ∈ Finset.range p, s e) - ∑ e ∈ Finset.range p, b e := by
      rw [← Finset.sum_tsub_distrib]; intro e he; simp only [Finset.mem_range] at he; exact hbs e (by omega)
...
theorem rhs_reindex (d : ℕ) (m : Fin d → ℕ) :
    ∑ i : Fin d, (i.val + 1) * m i
      = ∑ c ∈ Finset.range d, ∑ i ∈ Finset.univ.filter (fun i : Fin d => d - 1 - c ≤ i.val), m i := by
...
theorem slot_lower_bound (q d k : ℕ) (hq : 2 ≤ q) (hk : 0 < k)
  ...
  · -- hm_len
    ...
    rw [hk1k] at hb
    have hLval : Ls = k + d * (k + 1) + 1 := by simp only [hLs, slotLen]
...
theorem slotList_count (q k1 L : ℕ) (hq : 2 ≤ q) (e : ℕ) (he : e < L) :
  ...
  show ((List.range L).map (List.count (q^e) ∘ (fun e' => List.replicate ((q - 1) - qdigit q k1 e') (q ^ e')))).sum = _
...
theorem slot_upper_bound (q d k : ℕ) (hq : 2 ≤ q) (hk : 0 < k) :
  ...
  have hlen : d * (q - 1) ≤ (slotList q k1 L).length := by
    ...
    have hk1k : k1 + 1 = k := by omega
    rw [hk1k] at hb
    have hLval : L = k + d * (k + 1) + 1 := by simp only [hLdef, slotLen]
    have hdd : d ≤ d * (k + 1) := Nat.le_mul_of_pos_right d (by omega)
  ...
  have hmwin : ∀ i : Fin d, m i = (((slotList q k1 L).drop ((d - 1 - i.val) * (q-1))).take (q-1)).sum := by
    intro i
    have hi : i.val < d := i.isLt
  ...
  have hWpow : ∀ i : Fin d, ∀ x ∈ (((slotList q k1 L).drop ((d - 1 - i.val) * (q-1))).take (q-1)), ∃ e, e < L ∧ x = q^e := by
    intro i x hx
    have hsub : x ∈ slotList q k1 L := by
      have h1 : x ∈ (slotList q k1 L).drop ((d - 1 - i.val) * (q-1)) := List.mem_of_mem_take hx
      exact List.mem_of_mem_drop h1
    exact slotList_pow q k1 L x hsub
  -- window count ≤ slotList count
  have hWcount : ∀ i : Fin d, ∀ x, (((slotList q k1 L).drop ((d - 1 - i.val) * (q-1))).take (q-1)).count x ≤ (slotList q k1 L).count x := by
    intro i x
    apply List.Sublist.count_le
    exact (List.take_sublist _ _).trans (List.drop_sublist _ _)
  -- window length is q-1
  have hWlen : ∀ i : Fin d, (((slotList q k1 L).drop ((d - 1 - i.val) * (q-1))).take (q-1)).length = q-1 := by
    intro i; exact window_len _ _ _ (hoff i)
  -- carry-free per position e: total count over coordinates ≤ count in slotList
  have hcountsum : ∀ e, (∑ i : Fin d, (((slotList q k1 L).drop ((d - 1 - i.val) * (q-1))).take (q-1)).count (q^e))
      ≤ (slotList q k1 L).count (q^e) := by
    intro e
    -- reindex i ↦ s = d-1-i.val to range d, use take_blocks_count
    have hreindex : (∑ i : Fin d, (((slotList q k1 L).drop ((d - 1 - i.val) * (q-1))).take (q-1)).count (q^e))
        = ∑ s ∈ Finset.range d, (((slotList q k1 L).drop (s * (q-1))).take (q-1)).count (q^e) := by
      rw [Fin.sum_univ_eq_sum_range (fun i => (((slotList q k1 L).drop ((d - 1 - i) * (q-1))).take (q-1)).count (q^e)) d]
  ...
  have hmdig : ∀ (i : Fin d) (e : ℕ), qdigit q (m i) e
      = if e < L then (((slotList q k1 L).drop ((d - 1 - i.val) * (q-1))).take (q-1)).count (q^e) else 0 := by
    intro i e
    rw [hmwin i]
    ...
    · -- AddNoCarry
      ...
      have hsum : ∑ i : Fin d, qdigit q (m i) e
        ...
        · have heq : ∀ i : Fin d, qdigit q (m i) e
              = (((slotList q k1 L).drop ((d - 1 - i.val) * (q-1))).take (q-1)).count (q^e) := by
...
theorem slotList_drop (q k1 p r : ℕ)
  ...
  · push_neg at hLx
    ...
    have hrange : List.range Lx = List.range (p+1) ++ (List.range (Lx - (p+1))).map ((p+1) + ·) := by
    ...
    have hLHS : slotList q k1 Lx = (slotList q k1 p ++ List.replicate ((q-1)-qdigit q k1 p) (q^p))
...
theorem newmult_eq (q k1 p r : ℕ) (hq : 2 ≤ q)
    ...
    (q-1) - qdigit q (k1 + ((slotList q k1 p).sum + r * q^p)) e
      = if e < p then 0 else if e = p then ((q-1)-qdigit q k1 p) - r else (q-1)-qdigit q k1 e := by
  set β := (slotList q k1 p).sum + r * q^p with hβ
...
theorem slot_subtraction (q k1 L₀ : ℕ) (hq : 2 ≤ q)
  ...
  have hnew : slotList q (k1 + ((slotList q k1 p).sum + r * q^p)) Lx
    ...
    rw [show slotList q (k1 + ((slotList q k1 p).sum + r * q^p)) Lx
          = (List.range Lx).flatMap (fun e =>
              List.replicate ((q-1)-qdigit q (k1 + ((slotList q k1 p).sum + r * q^p)) e) (q^e)) from rfl]
    apply List.flatMap_congr
    intro e _
...
theorem layer_cake (q d k : ℕ) (hq : 2 ≤ q) (hd : 1 < d) (hk : 0 < k) :
  ...
  have hβ_one : blockSum q k1 (slotLen q 1 k) 1 = blockSum q k1 L 1 := by
    ...
    · rw [hLdef]; unfold slotLen; nlinarith [hk]
    · omega
    · have hb := slotList_length_ge q k1 (slotLen q 1 k) hq
      rw [hk1k] at hb
      have hge : 1 ≤ slotLen q 1 k - k := by unfold slotLen; omega
  ...
  have hlenL' : (d - 1) * (q - 1) ≤ (slotList q (s1 - 1) L').length := by
    ...
    rw [hLk] at hb
    have hL'val : L' = s1 + (d - 1) * (s1 + 1) + 1 := by simp only [hL'def, slotLen]
...
theorem removal_bound (q k1 u Lbig : ℕ) (hq : 2 ≤ q)
  ...
  have hMlen : t + m ≤ (slotList q k1 Lbig).length := by
    ...
    have hpt : (slotList q (k1+u) Lbig).length = ∑ e ∈ Finset.range Lbig, ((q-1) - qdigit q (k1+u) e) := slotList_length q (k1+u) Lbig
    rw [hpt] at ht
  ...
  have hcandval : ∑ e ∈ Finset.range Lbig, bcoef e * q^e = Tp.sum + u := by
    ...
    have h2 : ∑ e ∈ Finset.range Lbig, qdigit q u e * q^e = u := (digitsum_eq q u Lbig hq huL).symm
    rw [hbcoef]
    simp only []
...
theorem extraction_core (q : ℕ) (hq : 2 ≤ q) (k1 : ℕ) (d : ℕ) (hd : 1 < d)
  ...
  have hStepI : ∑ c ∈ Finset.range (d-1), P (c + 1 + ℓ)
    ...
    calc ∑ c ∈ Finset.range (d-1), P (c + 1 + ℓ)
        ≤ ∑ c ∈ Finset.range (d-1), (Q (c + 1) + u) := Finset.sum_le_sum (fun c hc => hA1 c (Finset.mem_range.mp hc))
  ...
  have hStepII : ∑ c ∈ Finset.range d, P (c + 1) ≤ (∑ c ∈ Finset.range (d-1), P (c + 1 + ℓ)) + P 1 := by
    ...
    have hsplitL : ∑ c ∈ Finset.range d, P (c + 1)
      ...
      rw [Nat.add_comm]
    rw [hsplitL]
    have hmonosum : ∑ c ∈ Finset.range (d-1), P (c + 2) ≤ ∑ c ∈ Finset.range (d-1), P (c + 1 + ℓ) :=
  ...
  have hkey1 : (∑ c ∈ Finset.range (d-1), P (c + 1 + ℓ)) ≤ Phi q (k1 + u) (d - 1) L₂ + (d - 1) * u := hStepI
  -- and StepII + hP1
...
theorem extraction_strict (q : ℕ) (hq : q.Prime) (d k : ℕ) (hd : 1 < d)
  ...
  have hL₂ : (d - 1) * (q - 1)
    ...
    have hsl : slotLen q (d - 1) (s1 + j) - (((k - 1) + u) + 1)
        = (d - 1) * ((k + u) + 1) + 1 := by
      simp only [slotLen]; rw [hnval]; omega
    rw [hsl] at hb

...
theorem negChoose_cast_ne_zero_iff (Fq : Type) [Field Fq]
  ...
  have bridge : (∀ i ∈ Finset.Ico 1 b, ¬ (p^i ≤ y % p^i + (k-1) % p^i)) ↔
    ...
    · intro H i hi hle
      ...
      have := H i hi.1; omega
  rw [bridge]
  show (∀ i, 1 ≤ i → y % p^i + (k-1) % p^i < p^i) ↔ CarryFree p [k-1, y]
...
theorem multinomial_cast_ne_zero_iff (Fq : Type) [Field Fq]
  ...
  have hrec : ∀ (a : ℕ) (L : List ℕ),
    ...
    rw [hc]
    have e1 : (a + L.sum).choose a * (a.factorial * ((L.map Nat.factorial).prod * c)) / (a.factorial * (L.map Nat.factorial).prod) = (a + L.sum).choose a * c := by
      rw [show (a + L.sum).choose a * (a.factorial * ((L.map Nat.factorial).prod * c)) = ((a + L.sum).choose a * c) * (a.factorial * (L.map Nat.factorial).prod) by ring]
  ...
  have crux_div : ∀ (a b : ℕ), 1 < p → (∀ e, digit p a e + digit p b e ≤ p - 1) →
    ...
    | succ e ih =>
      ...
      have key : (a / p ^ e + b / p ^ e) = p * (a / p ^ e / p + b / p ^ e / p) + ((a / p ^ e) % p + (b / p ^ e) % p) := by
    ...
    · interval_cases p
      · intro h e
        ...
        · unfold digit
          ...
          simp only [h0, Nat.div_zero, Nat.zero_mod, List.map_const', List.sum_replicate, smul_eq_mul, mul_zero]
      · intro h e
    ...
    · have hp' : 1 < p := hpp
      ...
      | cons a t ih =>
        ...
        have hpair : ∀ e, digit p a e + digit p t.sum e ≤ p - 1 := by
          intro e; rw [ihd e]; have := h e; simp only [List.map_cons, List.sum_cons] at this; exact this
  ...
  have hmain : ∀ L : List ℕ,
    ...
    | cons a t ih =>
      ...
      have hcast : (((a + t.sum).factorial / ((a :: t).map Nat.factorial).prod : ℕ) : Fq)
          = ((a + t.sum).choose a : Fq) * ((t.sum.factorial / (t.map Nat.factorial).prod : ℕ) : Fq) := by
      ...
      rw [hb]
      rw [show (((t.sum.factorial / (t.map Nat.factorial).prod : ℕ) : Fq)) = ((((t.sum.factorial / (t.map Nat.factorial).prod : ℕ) : ℤ)) : Fq) by rw [Int.cast_natCast]] at *
      rw [ih]
      rw [hcons a t]
  -- Reduce `Nat.multinomial Finset.univ m` to the list version over `List.ofFn m`.
  have hreduce : ((Nat.multinomial Finset.univ m : ℤ) : Fq)
      = ((((List.ofFn m).sum.factorial / ((List.ofFn m).map Nat.factorial).prod : ℕ) : ℤ) : Fq) := by
...
theorem coeff_inv_one_add_pow
  ...
  have coeff_pow_high : ∀ d : ℕ, e < d → (PowerSeries.coeff (R := Fq) e) (g^d) = 0 := by
    ...
    calc (e : ℕ∞) < d := by exact_mod_cast h
      _ ≤ (g^d).order := h2
  have coeff_neg_pow : ∀ d : ℕ, (PowerSeries.coeff (R := Fq) e) ((-g)^d) = (-1)^d * (PowerSeries.coeff (R := Fq) e) (g^d) := by
    intro d
  ...
  · obtain ⟨k', rfl⟩ := Nat.exists_eq_add_of_lt hk
    ...
    have hinv : (((1 + g) ^ (k'+1))⁻¹) =
      ...
      have h1 : S ((1 - PowerSeries.X)^(k'+1)) = (1 + g)^(k'+1) := by
        rw [map_pow, map_sub, map_one, hS, PowerSeries.coe_substAlgHom, PowerSeries.subst_X ha]; ring
      rw [h1] at happ
    ...
    rw [finsum_eq_finset_sum_of_support_subset _ (s := Finset.range (e+1))]
...
theorem phi_monicOf_eq (Fq : Type) [Field Fq] (d : ℕ) (θ : Fin d → Fq) :
    phi Fq (monicOf Fq d θ)
      = (HahnSeries.single (-(d : ℤ)) 1)
          * (HahnSeries.ofPowerSeries ℤ Fq
              (1 + ∑ i : Fin d, (PowerSeries.C (R := Fq) (θ i)) * PowerSeries.X ^ (d - (i : ℕ)))) := by
  have hpow : ∀ (n : ℕ) (a : ℤ), (HahnSeries.single a (1:Fq))^n = HahnSeries.single (n * a) 1 := by
    intro n a
    induction n with
    | zero => simp
    | succ k ih => rw [pow_succ, ih, HahnSeries.single_mul_single]; congr 1 <;> push_cast <;> ring
  have hXinv : ((PowerSeries.X : PowerSeries Fq) : LaurentSeries Fq)⁻¹ = HahnSeries.single (-1) 1 := by
  ...
  have hofX : ∀ m : ℕ, HahnSeries.ofPowerSeries ℤ Fq (PowerSeries.X ^ m) = HahnSeries.single (m : ℤ) 1 := by
    intro m
  ...
  have hRHSterm : ∀ i : Fin d,
      (HahnSeries.single (-(d : ℤ)) (1:Fq)) *
          (HahnSeries.ofPowerSeries ℤ Fq ((PowerSeries.C (R := Fq) (θ i)) * PowerSeries.X ^ (d - (i : ℕ))))
        = HahnSeries.single (-(i : ℤ)) (θ i) := by
    intro i
...
theorem inv_phi_pow_eq_shift_mul (Fq : Type) [Field Fq] (d k : ℕ) (θ : Fin d → Fq) :
    (phi Fq (monicOf Fq d θ))⁻¹ ^ k
      = (HahnSeries.single (d * k : ℤ) 1)
          * (HahnSeries.ofPowerSeries ℤ Fq
              (((1 + ∑ i : Fin d, (PowerSeries.C (R := Fq) (θ i)) * PowerSeries.X ^ (d - (i : ℕ))) ^ k)⁻¹)) := by
...
theorem coeff_single_mul_coe_powerSeries
    ...
    ((HahnSeries.single s 1 * (HahnSeries.ofPowerSeries ℤ Fq f)) : LaurentSeries Fq).coeff n
      = if h : 0 ≤ n - s then (PowerSeries.coeff (R := Fq) (n - s).toNat) f else 0 := by
...
theorem Sdk_coeff_per_theta (Fq : Type) [Field Fq] (d k : ℕ) (θ : Fin d → Fq) (n : ℤ) :
  ...
  · next h =>
    ...
    have hcoeff : ∀ y : ℕ, (PowerSeries.coeff (R := Fq) e) (g ^ y) =
      ...
      apply Finset.sum_congr rfl
      intro c hc
      have hprod : (∏ i : Fin d, ((PowerSeries.C (R:=Fq) (θ i)) * PowerSeries.X ^ (d - (i:ℕ)))^(c i))
          = PowerSeries.C (∏ i : Fin d, (θ i)^(c i)) * PowerSeries.X ^ (∑ i : Fin d, (d - (i:ℕ)) * c i) := by
        have step : ∀ i : Fin d, ((PowerSeries.C (R:=Fq) (θ i)) * PowerSeries.X ^ (d - (i:ℕ)))^(c i)
      ...
      rw [hprod]
      rw [show ((Nat.multinomial Finset.univ c : PowerSeries Fq)) = PowerSeries.C (Nat.multinomial Finset.univ c : Fq) by simp]
...
theorem Sdk_coeff_eq_finsum (Fq : Type) [Field Fq] [Fintype Fq]
    (p f d k : ℕ) (hp : p.Prime) (hf : 1 ≤ f) (hd : 1 ≤ d) (hk : 0 < k)
    (hchar : CharP Fq p) (hq : Fintype.card Fq = p ^ f) (n : ℤ) :
    (Sdk Fq d k).coeff n =
  ...
  have hsetT : {m : Fin d → ℕ | Tindex p (Fintype.card Fq) d (k - 1) m ∧
        ((d * k : ℕ) + weight d m : ℤ) = n} = ↑(FS.filter (Tindex p (Fintype.card Fq) d (k-1))) := by
    ext m
  ...
  rw [show (∏ i, (∑ x : Fq, x ^ (m i))) =
    ...
    · rw [if_neg h]; push_neg at h; obtain ⟨i, hi⟩ := h
...
theorem monicOf_surjective (Fq : Type) [Field Fq] (d : ℕ) (a : Polynomial Fq)
  ...
  show X ^ d + ∑ i : Fin d, C (a.coeff (i : ℕ)) * X ^ (i : ℕ) = a
  rw [hfin]

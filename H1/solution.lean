...
lemma F_lt_of_Phi_lt (q d k : ℕ) (hq : 2 ≤ q)
    (kk kk' : Fin (d + 1) → ℕ)
      ...
      ((q : ℤ) - 1) * (∑ i : Fin (d + 1), (tt i : ℤ) * bCoeff q (i : ℕ) d)
        = (q : ℤ) * (∑ i : Fin (d + 1), (tt i : ℤ) * (q : ℤ) ^ (i : ℕ))
          - ((q : ℤ) ^ (d + 1) * (∑ i : Fin (d + 1), (tt i : ℤ))
            + ((q : ℤ) - 1) * ∑ i : Fin (d + 1), (i : ℤ) * (q : ℤ) ^ (i : ℕ) * (tt i)) := by
    intro tt
    rw [Finset.mul_sum]
    rw [Finset.mul_sum (s := (Finset.univ : Finset (Fin (d+1)))) (f := fun i => (tt i : ℤ) * (q : ℤ) ^ (i : ℕ))]
    rw [Finset.mul_sum (s := (Finset.univ : Finset (Fin (d+1)))) (f := fun i => (tt i : ℤ))]
    rw [Finset.mul_sum (s := (Finset.univ : Finset (Fin (d+1)))) (f := fun i => (i : ℤ) * (q : ℤ) ^ (i : ℕ) * (tt i))]
    rw [show (∑ i : Fin (d+1), (q : ℤ) * ((tt i : ℤ) * (q : ℤ) ^ (i : ℕ)))
          - ((∑ i : Fin (d+1), (q : ℤ) ^ (d + 1) * (tt i : ℤ))
            + ∑ i : Fin (d+1), ((q : ℤ) - 1) * ((i : ℤ) * (q : ℤ) ^ (i : ℕ) * (tt i)))
        = ∑ i : Fin (d+1), ((q : ℤ) * ((tt i : ℤ) * (q : ℤ) ^ (i : ℕ))
            - ((q : ℤ) ^ (d + 1) * (tt i : ℤ) + ((q : ℤ) - 1) * ((i : ℤ) * (q : ℤ) ^ (i : ℕ) * (tt i)))) by
  ...
  have hPhiZ : ((q : ℤ) ^ (d + 1) * ∑ i : Fin (d + 1), (kk' i : ℤ)
    ...
    convert hcastineq using 2 <;> ring
  -- now finish
  unfold F
...
unchanged. -/
theorem repr_preserved (q d m : ℕ) (hq : 1 ≤ q) (kk : Fin (d+1) → ℕ)
    (r : Fin (d+1) → ℕ) (jj : Fin (d+1)) (j : ℕ)
    (hjj : (jj:ℕ) = j + 1) (hjm : j ≤ m)
    (hrle : ∀ i : Fin (d+1), r i ≤ digit q (kk i) (m - (i:ℕ)))
    (hr0 : ∀ i : Fin (d+1), m < (i:ℕ) → r i = 0)
    (hrsum : (∑ i, r i) = q) :
    (∑ i : Fin (d+1), (kk i - r i * q^(m - (i:ℕ)) + (if i = jj then q^(m - j) else 0)) * q^(i:ℕ))
  ...
  have hkey : (∑ i : Fin (d+1), (kk i - r i * q^(m - (i:ℕ)) + (if i = jj then q^(m - j) else 0)) * q^(i:ℕ))
        + ∑ i : Fin (d+1), (r i * q^(m - (i:ℕ))) * q^(i:ℕ)
      = (∑ i : Fin (d+1), kk i * q^(i:ℕ)) + ∑ i : Fin (d+1), (if i = jj then q^(m - j) else 0) * q^(i:ℕ) := by
    rw [← Finset.sum_add_distrib, ← Finset.sum_add_distrib]
    apply Finset.sum_congr rfl
    intro i _
    exact peridx (kk i) (r i * q^(m-(i:ℕ))) (if i = jj then q^(m-j) else 0) (q^(i:ℕ)) (hno i)
  have hsum1 : (∑ i : Fin (d+1), (r i * q^(m - (i:ℕ))) * q^(i:ℕ)) = q^(m+1) := by
    have heq : ∑ i : Fin (d+1), (r i * q^(m - (i:ℕ))) * q^(i:ℕ) = q^m * ∑ i, r i := by
      rw [Finset.mul_sum]
      apply Finset.sum_congr rfl
      intro i _
      by_cases hi : (i:ℕ) ≤ m
      · have hpp : q^(m - (i:ℕ)) * q^(i:ℕ) = q^m := by rw [← pow_add]; congr 1; omega
        rw [mul_assoc, hpp, Nat.mul_comm]
      · push_neg at hi; simp [hr0 i hi]
    rw [heq, hrsum, pow_succ]
  have hsum2 : (∑ i : Fin (d+1), (if i = jj then q^(m - j) else 0) * q^(i:ℕ)) = q^(m+1) := by
...
theorem row_select_j (q d : ℕ) (hq : 2 ≤ q) (r : Fin (d+1) → ℕ)
  ...
  have hSne : S.Nonempty := by
    ...
    have hall : ∀ i : Fin (d+1), (i:ℕ) < d → r i = 0 := by
      intro i hi
      by_contra hr
      have : i ∈ S := by rw [hS]; simp; exact ⟨hi, Nat.pos_of_ne_zero hr⟩
  ...
  intro i hlt hid
  by_contra hr
  have hiS : i ∈ S := by rw [hS]; simp; exact ⟨hid, Nat.pos_of_ne_zero hr⟩
...
lemma exchange_carryfree (q d m : ℕ) (hq2 : 2 ≤ q) (kk : Fin (d + 1) → ℕ)
    ...
    (hjv : jv = (jrow : ℕ)) (hjjval : (jj : ℕ) = jv + 1) (hjm : jv ≤ m)
...
lemma exchange_phi_gain (q d k m : ℕ) (hq2 : 2 ≤ q) (kk : Fin (d + 1) → ℕ)
    ...
    (hrsum : (∑ i, r i) = q)
    (hjpos : 0 < r jrow)
    (hjtop : ∀ i : Fin (d + 1), jv < (i : ℕ) → (i : ℕ) < d → r i = 0)
    (kk' : Fin (d + 1) → ℕ)
    (hkk' : ∀ i, kk' i = kk i - r i * q ^ (m - (i : ℕ)) + (if i = jj then q ^ (m - jv) else 0)) :
    F q d kk < F q d kk' := by
  have hq1 : 1 ≤ q := by omega
  ...
  have hlb : gWeight q d jv + (q - 1) * gWeight q d d
    ...
    have hkey2 : gWeight q d jv + (q - 1) * gWeight q d d
      ...
      omega
    rw [hsplit]
    -- hrest : g_d * (∑_{erase} r) ≤ ∑_{erase} g_i r_i ; hsumrest : ∑_{erase} r = q - r jrow
...
lemma carry_exchange_improves (q d k : ℕ) (hq : q.Prime) (kk : Fin (d + 1) → ℕ)
  ...
  -- diagonal counts
  set a : Fin (d+1) → ℕ := fun i => if (i:ℕ) ≤ m then digit q (kk i) (m - (i:ℕ)) else 0 with ha
...
lemma move_parallelogram (q d k m i j t : ℕ) (hq : 2 ≤ q) (kk : Fin (d + 1) → ℕ)
  ...
  have hIJ : I ≠ J := by intro h; have := congrArg Fin.val h; simp [hIval, hJval] at this; omega
  have hIIT : I ≠ IT := by intro h; have := congrArg Fin.val h; simp [hIval, hITval] at this; omega
  have hIJT : I ≠ JT := by intro h; have := congrArg Fin.val h; simp [hIval, hJTval] at this; omega
  have hJJT : J ≠ JT := by intro h; have := congrArg Fin.val h; simp [hJval, hJTval] at this; omega
  have hITJT : IT ≠ JT := by intro h; have := congrArg Fin.val h; simp [hITval, hJTval] at this; omega
  ...
  have hkkI : kk' I = kk I - q ^ (m - i) := by
    simp only [hkk'def, if_pos rfl, if_neg hIJT, if_neg hIIT, if_neg hIJ, if_true, add_zero,
      Nat.sub_zero]
  have hkkJT : kk' JT = kk JT - q ^ (m - j) := by
    simp only [hkk'def, if_neg hIJT.symm, if_pos rfl, if_neg hITJT.symm, if_neg hJJT.symm, if_true,
      add_zero, Nat.sub_zero]
  refine ⟨kk', ⟨?_, ?_⟩, ?_⟩
  · -- representation
    rw [hrep]
    -- per-row no-borrow bound: P_r + S_r ≤ kk r
    have hPS : ∀ r : Fin (d + 1),
        (if r = I then q ^ (m - i) else 0) + (if r = JT then q ^ (m - j) else 0) ≤ kk r := by
      intro r
      by_cases hrI : r = I
    ...
    -- the balance: ∑ kk' r q^r + ∑ (P+S) q^r = ∑ kk r q^r + ∑ (Q+R) q^r
    have hbal : (∑ r : Fin (d + 1), kk' r * q ^ (r : ℕ))
      ...
      -- (kk r - P - S + Q + R) * q^r + (P+S)*q^r = kk r * q^r + (Q+R)*q^r
      have hsub : kk r - P - S + Q + R = kk r - (P + S) + (Q + R) := by omega
      rw [hsub]
      have e1 : (kk r - (P + S) + (Q + R)) * q ^ (r:ℕ)
          = (kk r - (P + S)) * q ^ (r:ℕ) + (Q + R) * q ^ (r:ℕ) := by rw [Nat.add_mul]
      have e2 : (kk r - (P + S)) * q ^ (r:ℕ) = kk r * q ^ (r:ℕ) - (P + S) * q ^ (r:ℕ) := by
        rw [Nat.sub_mul]
      rw [e1, e2]
      have hle : (P + S) * q ^ (r : ℕ) ≤ kk r * q ^ (r : ℕ) := Nat.mul_le_mul_right _ hps
      omega
    -- evaluate the four single-row sums
    have hsumI : (∑ r : Fin (d + 1), (if r = I then q ^ (m - i) else 0) * q ^ (r : ℕ)) = q ^ m := by
      rw [Finset.sum_eq_single I]
      · rw [if_pos rfl, hIval, ← pow_add]; congr 1; omega
      · intro b _ hb; rw [if_neg hb]; ring
      · intro h; exact absurd (Finset.mem_univ _) h
    have hsumJT : (∑ r : Fin (d + 1), (if r = JT then q ^ (m - j) else 0) * q ^ (r : ℕ)) = q ^ (m + t) := by
      rw [Finset.sum_eq_single JT]
      · rw [if_pos rfl, hJTval, ← pow_add]; congr 1; omega
      · intro b _ hb; rw [if_neg hb]; ring
      · intro h; exact absurd (Finset.mem_univ _) h
    have hsumIT : (∑ r : Fin (d + 1), (if r = IT then q ^ (m - i) else 0) * q ^ (r : ℕ)) = q ^ (m + t) := by
      rw [Finset.sum_eq_single IT]
      · rw [if_pos rfl, hITval, ← pow_add]; congr 1; omega
      · intro b _ hb; rw [if_neg hb]; ring
      · intro h; exact absurd (Finset.mem_univ _) h
    have hsumJ : (∑ r : Fin (d + 1), (if r = J then q ^ (m - j) else 0) * q ^ (r : ℕ)) = q ^ m := by
    ...
    have hL : (∑ r : Fin (d + 1), ((if r = I then q ^ (m - i) else 0)
              + (if r = JT then q ^ (m - j) else 0)) * q ^ (r : ℕ)) = q ^ m + q ^ (m + t) := by
      rw [show (∑ r : Fin (d + 1), ((if r = I then q ^ (m - i) else 0)
              + (if r = JT then q ^ (m - j) else 0)) * q ^ (r : ℕ))
            = (∑ r : Fin (d + 1), (if r = I then q ^ (m - i) else 0) * q ^ (r : ℕ))
              + ∑ r : Fin (d + 1), (if r = JT then q ^ (m - j) else 0) * q ^ (r : ℕ) from by
              rw [← Finset.sum_add_distrib]; apply Finset.sum_congr rfl; intro r _; rw [Nat.add_mul]]
      rw [hsumI, hsumJT]
    have hRR : (∑ r : Fin (d + 1), ((if r = IT then q ^ (m - i) else 0)
              + (if r = J then q ^ (m - j) else 0)) * q ^ (r : ℕ)) = q ^ (m + t) + q ^ m := by
      rw [show (∑ r : Fin (d + 1), ((if r = IT then q ^ (m - i) else 0)
              + (if r = J then q ^ (m - j) else 0)) * q ^ (r : ℕ))
            = (∑ r : Fin (d + 1), (if r = IT then q ^ (m - i) else 0) * q ^ (r : ℕ))
              + ∑ r : Fin (d + 1), (if r = J then q ^ (m - j) else 0) * q ^ (r : ℕ) from by
              rw [← Finset.sum_add_distrib]; apply Finset.sum_congr rfl; intro r _; rw [Nat.add_mul]]
      rw [hsumIT, hsumJ]
    rw [hL, hRR] at hbal
    omega
  · -- carry-free preservation
    -- precondition facts from the source diagonal constraints
    -- position m-i: I contributes ≥ 1, so any other single row's digit there is ≤ q-2
    have hcfI : ∀ r : Fin (d + 1), r ≠ I → digit q (kk r) (m - i) + 1 < q := by
      ...
      omega
    -- position m-j: JT contributes ≥ 1, so any other single row's digit there is ≤ q-2
    have hcfJ : ∀ r : Fin (d + 1), r ≠ JT → digit q (kk r) (m - j) + 1 < q := by
      ...
      have hpair : digit q (kk r) (m - j) + digit q (kk JT) (m - j)
        ...
        rwa [Finset.sum_pair hr] at this
      omega
    -- characterization of kk' on the always-distinct rows at any position n
    ...
    · -- collision case: IT = J
      ...
      -- kk' J = kk J + q^(m-i) + q^(m-j)  (it is IT and J)
      have hkkJcol : kk' J = kk J + q ^ (m - i) + q ^ (m - j) := by
        have e : kk' J = kk J - (if J = I then q ^ (m - i) else 0)
              - (if J = JT then q ^ (m - j) else 0)
              + (if J = IT then q ^ (m - i) else 0) + (if J = J then q ^ (m - j) else 0) := rfl
        rw [e, if_neg hIJ.symm, if_neg hJJT, if_pos hITJ.symm, if_pos rfl,
          Nat.sub_zero, Nat.sub_zero]
      -- digit of kk' J: at m-i it is +1, at m-j it is +1, else unchanged
      have hdigJ : ∀ n : ℕ, digit q (kk' J) n
          = if n = m - i then digit q (kk J) n + 1
            else if n = m - j then digit q (kk J) n + 1 else digit q (kk J) n := by
        intro n
        rw [hkkJcol]
        have hpre_i : digit q (kk J) (m - i) + 1 < q := hcfI J (by rw [← hITJ]; exact hIIT.symm)
        have hpre_j : digit q (kk J) (m - j) + 1 < q := hcfJ J hJJT
        have hmid_j : digit q (kk J + q ^ (m-i)) (m-j) + 1 < q := by
          rw [digit_add_other q (kk J) (m-i) (m-j) hq1 hpre_i hmimj.symm]; exact hpre_j
        by_cases hni : n = m - i
        · subst hni
          rw [if_pos rfl,
            digit_add_other q (kk J + q ^ (m-i)) (m-j) (m-i) hq1 hmid_j hmimj,
            digit_add_self q (kk J) (m-i) hq1 hpre_i]
        · by_cases hnj : n = m - j
          · subst hnj
            rw [if_neg hni, if_pos rfl,
              digit_add_self q (kk J + q^(m-i)) (m-j) hq1 hmid_j,
              digit_add_other q (kk J) (m-i) (m-j) hq1 hpre_i hni]
      ...
      have hkkO : ∀ r : Fin (d + 1), r ≠ I → r ≠ J → r ≠ JT → kk' r = kk r := by
        ...
        simp only [hkk'def, if_neg hrI, if_neg hrJ, if_neg hrJT, if_neg hrIT, Nat.sub_zero, add_zero]
      -- sum over the 3 touched rows {I, J, JT}
      intro n
      ...
      have heq : (∑ r : Fin (d + 1), digit q (kk' r) n)
        ...
        -- now an omega over the if-cases
        by_cases hni : n = m - i
        · subst hni
      ...
      rw [heq]; exact hcf n
    · -- non-collision case: IT ≠ J, 4 distinct rows
      have hITJ : IT ≠ J := by intro h; have := congrArg Fin.val h; simp [hITval, hJval] at this; omega
      have hkkIT : kk' IT = kk IT + q ^ (m - i) := by
        have e : kk' IT = kk IT - (if IT = I then q ^ (m - i) else 0)
              - (if IT = JT then q ^ (m - j) else 0)
              + (if IT = IT then q ^ (m - i) else 0) + (if IT = J then q ^ (m - j) else 0) := rfl
  ...
  · -- Phi q d kk' < Phi q d kk
    ...
    have hbal : (∑ r : Fin (d + 1), w r * kk' r)
      ...
      have hsub : kk r - P - S + Q + R = kk r - (P + S) + (Q + R) := by omega
      rw [hsub]
      have e1 : w r * (kk r - (P + S) + (Q + R)) = w r * (kk r - (P + S)) + w r * (Q + R) := by
        rw [Nat.mul_add]
      have e2 : w r * (kk r - (P + S)) = w r * kk r - w r * (P + S) := by rw [Nat.mul_sub]
      rw [e1, e2]
      have hle : w r * (P + S) ≤ w r * kk r := Nat.mul_le_mul_left _ hps
      omega
    -- evaluate the four single-row weighted sums
    ...
    have hswJ : (∑ r : Fin (d + 1), w r * (if r = J then q ^ (m - j) else 0))
      ...
      · intro b _ hb; rw [if_neg hb, Nat.mul_zero]
      · intro h; exact absurd (Finset.mem_univ _) h
    -- combine the pair-sums into the balance
    have hLb : (∑ r : Fin (d + 1), w r * ((if r = I then q ^ (m - i) else 0)
      ...
      rw [show (∑ r : Fin (d + 1), w r * ((if r = I then q ^ (m - i) else 0)
            ...
            = (∑ r : Fin (d + 1), w r * (if r = I then q ^ (m - i) else 0))
              ...
              rw [← Finset.sum_add_distrib]; apply Finset.sum_congr rfl; intro r _; rw [Nat.mul_add]]
      rw [hswI, hswJT]
    ...
    have hppit : q ^ (i + t) * q ^ (m - i) = q ^ (m + t) := by rw [← pow_add]; congr 1; omega
    have hppj : q ^ j * q ^ (m - j) = q ^ m := by rw [← pow_add]; congr 1; omega
    have hppjt : q ^ (j + t) * q ^ (m - j) = q ^ (m + t) := by rw [← pow_add]; congr 1; omega
    -- expand the four weighted terms
    have hwIv : w I * q ^ (m - i) = q ^ (d + 1) * q ^ (m - i) + (q - 1) * i * q ^ m := by
      simp only [hw, hIval]
      calc (q ^ (d + 1) + (q - 1) * (i * q ^ i)) * q ^ (m - i)
          = q ^ (d + 1) * q ^ (m - i) + (q - 1) * i * (q ^ i * q ^ (m - i)) := by ring
        _ = q ^ (d + 1) * q ^ (m - i) + (q - 1) * i * q ^ m := by rw [hppi]
    have hwITv : w IT * q ^ (m - i) = q ^ (d + 1) * q ^ (m - i) + (q - 1) * (i + t) * q ^ (m + t) := by
      simp only [hw, hITval]
      calc (q ^ (d + 1) + (q - 1) * ((i + t) * q ^ (i + t))) * q ^ (m - i)
          = q ^ (d + 1) * q ^ (m - i) + (q - 1) * (i + t) * (q ^ (i + t) * q ^ (m - i)) := by ring
        _ = q ^ (d + 1) * q ^ (m - i) + (q - 1) * (i + t) * q ^ (m + t) := by rw [hppit]
    have hwJv : w J * q ^ (m - j) = q ^ (d + 1) * q ^ (m - j) + (q - 1) * j * q ^ m := by
      simp only [hw, hJval]
      calc (q ^ (d + 1) + (q - 1) * (j * q ^ j)) * q ^ (m - j)
          = q ^ (d + 1) * q ^ (m - j) + (q - 1) * j * (q ^ j * q ^ (m - j)) := by ring
        _ = q ^ (d + 1) * q ^ (m - j) + (q - 1) * j * q ^ m := by rw [hppj]
    have hwJTv : w JT * q ^ (m - j) = q ^ (d + 1) * q ^ (m - j) + (q - 1) * (j + t) * q ^ (m + t) := by
      simp only [hw, hJTval]
      calc (q ^ (d + 1) + (q - 1) * ((j + t) * q ^ (j + t))) * q ^ (m - j)
          = q ^ (d + 1) * q ^ (m - j) + (q - 1) * (j + t) * (q ^ (j + t) * q ^ (m - j)) := by ring
        _ = q ^ (d + 1) * q ^ (m - j) + (q - 1) * (j + t) * q ^ (m + t) := by rw [hppjt]
    -- the strict inequality: added < removed
    have hpowlt : q ^ m < q ^ (m + t) := by
      apply Nat.pow_lt_pow_right (by omega); omega
    have hcore : (j - i) * q ^ m < (j - i) * q ^ (m + t) := by
      have hji : 0 < j - i := by omega
      exact Nat.mul_lt_mul_of_pos_left hpowlt hji
    -- reduced inequality with small atoms
    have hred : (q - 1) * (i + t) * q ^ (m + t) + (q - 1) * j * q ^ m
      ...
      have hbase : (i + t) * Pt + j * Pm < i * Pm + (j + t) * Pt := by
        ...
        have hLe : (i + t) * Pt + (i + s) * Pm = (i * Pt + t * Pt + i * Pm) + s * Pm := by ring
        have hRe : i * Pm + (i + s + t) * Pt = (i * Pt + t * Pt + i * Pm) + s * Pt := by ring
        rw [hLe, hRe]
...
theorem digit_eq_of_repr' (q : ℕ) (hq : 2 ≤ q) :
  ...
  | succ m ih =>
    ...
    · subst hM
      ...
      rw [hcm, hdigrec, hrep]; simp [digit]
    · obtain ⟨M', rfl⟩ : ∃ M', M = M' + 1 := ⟨M - 1, by omega⟩
      have hdec : (∑ i ∈ Finset.range (M'+1), c i * q ^ i)
          = c 0 + q * ∑ i ∈ Finset.range M', c (i+1) * q ^ i := by
        rw [Finset.sum_range_succ', Finset.mul_sum, add_comm]
...
lemma column_sum_eq (q d k : ℕ) (hq : q.Prime) (kk : Fin (d + 1) → ℕ)
  ...
  have hcolrep : k = ∑ mm ∈ Finset.range M, colSum mm * q ^ mm := by
    ...
    have step2 : ∀ i : Fin (d+1),
        (∑ mm ∈ Finset.range M, (if (i:ℕ) ≤ mm then digit q (kk i) (mm - (i:ℕ)) * q ^ mm else 0))
          = kk i * q ^ (i:ℕ) := by
      intro i
